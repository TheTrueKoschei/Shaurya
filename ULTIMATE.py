...
def diverging_scale(kind):
    """
    Diverging colours for change maps, with a neutral centre that matches the
    theme. A white centre on a dark page reads as "no data"; a dark centre
    reads as "no change" and lets real change stand out.
    """
    mid = "#1b2733" if DK else "#eef1f4"
    if kind == "rainfall":           # drier = brown, wetter = teal
        ends = ["#8c510a", "#d8a657", mid, "#4fc3b0", "#00796b"]
    else:                            # cooler = blue, warmer = red
        ends = ["#2166ac", "#6fa8dc", mid, "#f08a5d", "#c0392b"]
    return [[0.0, ends[0]], [0.22, ends[1]], [0.5, ends[2]],
            [0.78, ends[3]], [1.0, ends[4]]]


def map_range(*zs, symmetric=None, robust=True):
    """
    Colour range across one or more grids.

    robust: use the 2nd-98th percentiles rather than min/max, so one extreme
    cell (a 40 mm/day coastal downpour) can't flatten everything else into
    one colour. The few cells beyond it show the end colour; hovering still
    ...
    if v.size == 0:
        return 0.0, 1.0
    if robust and v.size >= 50:
        lo, hi = (float(x) for x in np.percentile(v, [2, 98]))
        if lo == hi:
    ...
    if sym:
        m = (float(np.percentile(np.abs(v), 98)) if robust and v.size >= 50
             else max(abs(lo), abs(hi))) or 1.0
        return -m, m
    if lo == hi:
...
def map_figure(prep, variable, units, palette, vmin, vmax, height=440):
    ...
    if prep["coarse"]:
        ...
        # Heatmap, not Contour: at ~300x300 the result looks identical, but a
        # heatmap renders as one image while a contour is traced cell by cell
        # in the browser. With three maps on Compare that was the stutter.
        # Values rounded to 2 dp to shrink what is sent to the browser.
        xs = np.round(np.asarray(prep["x"], float), 4)
        ys = np.round(np.asarray(prep["y"], float), 4)
        if prep.get("focus"):
            # surroundings, faded, so the state sits in its real context
            fig.add_trace(go.Heatmap(z=np.round(prep["z_ctx"], 2), x=xs, y=ys,
                                     colorscale=palette, zmin=vmin, zmax=vmax,
                                     zsmooth=False, showscale=False, opacity=0.28,
                                     hoverinfo="skip"))
        fig.add_trace(go.Heatmap(z=np.round(prep["z"], 2), x=xs, y=ys,
                                 colorscale=palette, zmin=vmin, zmax=vmax,
                                 zsmooth=False, colorbar=cb, hovertemplate=hov))
...
def _state_geom(sig, name):
    ...
    # "object", so select by name rather than by dtype.
    text_cols = [c for c in gdf.columns if c != "geometry"]
...
# =================================================================
# DATASET LOADING — once per uploaded file, not once per click
# =================================================================
@st.cache_resource(show_spinner="Reading the NetCDF file\u2026", max_entries=3)
def _open_netcdf(file_key, _data):
    """
    Parse an uploaded NetCDF file once and keep it in memory.

    Previously every rerun (every slider move, every click) wrote the whole
    upload to a new temp file that was never deleted, then parsed it again.
    On a hosted server that is both slow and a slow leak of disk space.
    `_data` is skipped for hashing; `file_key` identifies the upload.
    """
    ...
    ds = None
    try:
        for kwargs in ({}, {"decode_times": False, "engine": "netcdf4"},
...
@st.cache_resource(show_spinner="Opening the dataset\u2026", max_entries=6)
def _open_netcdf_path(path, mtime):
    """Open a dataset that ships with the app. `mtime` refreshes the cache if
    the file is rebuilt."""
    for kwargs in ({}, {"decode_times": False, "engine": "netcdf4"},
                   {"decode_times": False, "engine": "h5netcdf"}):
        try:
            with xr.open_dataset(path, **kwargs) as f:
                return f.load()
        except Exception:
            continue
    return None
...
_SAMPLE_DIRS = [
    ("india_regions_ncep", "temperature, humidity & wind", "NCEP \u00b7 1948\u20132026"),
    ("india_regions",      "rainfall",                     "IMD \u00b7 2015\u20132023"),
]
_REGION_NAMES = {"All": "All India", "Indo Gangetic Plain": "Indo-Gangetic Plain",
                 "Western Ghats Konkan": "Western Ghats & Konkan"}


def sample_catalog():
    """Datasets bundled in the repo, so visitors can explore without a file."""
    out = []
    for folder, what, when in _SAMPLE_DIRS:
        d = os.path.join(_script_dir, folder)
        ...
        for f in sorted(os.listdir(d)):
            if not f.lower().endswith(".nc"):
                continue
            stem = f[:-3]
            if stem.upper().startswith("INDIA_"):
                stem = stem[6:]
            region = _REGION_NAMES.get(stem.replace("_", " "), stem.replace("_", " "))
            path = os.path.join(d, f)
            out.append(dict(label=f"{region} \u2014 {what}", region=region, what=what,
                            when=when, path=path, group=folder,
                            mtime=os.path.getmtime(path)))
    out.sort(key=lambda s: (s["group"] != "india_regions_ncep",
                            s["region"] != "All India", s["region"]))
    return out


def _set_source(src_dict):
    """Switch dataset. Resets the region choice, which may not fit the new file."""
    st.session_state["_src"] = src_dict
    if "region_choice" in st.session_state:
        st.session_state["region_choice"] = "Whole file"


def _use_sample(path):
    for s in sample_catalog():
        if s["path"] == path:
            _set_source(dict(kind="sample", key=f"{path}|{s['mtime']}",
                             name=s["label"], path=path, mtime=s["mtime"]))
            return


def _pick_sample_from(widget_key):
...
def _take_upload(widget_key):
    """Keep the upload's bytes ourselves: the uploader widget forgets its file
    as soon as it stops being drawn, e.g. when the landing page disappears."""
    f = st.session_state.get(widget_key)
    if f is None:
        return
    key = f"{getattr(f, 'file_id', '') or f.name}|{f.size}"
    cur = st.session_state.get("_src")
    if cur and cur.get("key") == key:
        return
    _set_source(dict(kind="upload", key=key, name=f.name, data=f.getvalue()))


with st.sidebar:

    _src = st.session_state.get("_src")
    _k = st.session_state.get("_file_uploader_key", 0)

    if _src is not None:
        with st.expander("\U0001F4C2  Dataset", expanded=True):
            st.markdown(
                f'<div style="font-size:0.66rem;letter-spacing:0.07em;text-transform:uppercase;'
                f'opacity:0.55;">Now showing</div>'
                f'<div style="font-size:0.9rem;font-weight:600;margin:2px 0 10px 0;'
                f'overflow-wrap:anywhere;">{_src["name"]}</div>',
                unsafe_allow_html=True)
            _samples = sample_catalog()
            if _samples:
                st.selectbox("Switch to a sample dataset",
                             ["Switch to a sample dataset\u2026"] + [x["label"] for x in _samples],
                             key=f"side_sample_{_k}", label_visibility="collapsed",
                             on_change=_pick_sample_from, args=(f"side_sample_{_k}",))
            st.file_uploader("Or upload another .nc file", type=["nc"],
                             key=f"nc_uploader_{_k}",
                             on_change=_take_upload, args=(f"nc_uploader_{_k}",))

    st.session_state.dataset_loaded = _src is not None

    if st.session_state.dataset_loaded:
        with st.expander("\U0001F321\ufe0f  Variable Selection"):
        ...
        with st.expander("\U0001F3A8  Colour Settings"):
    ...
    if _src is not None:
        try:
            if _src["kind"] == "upload":
                ds = _open_netcdf(_src["key"], _src["data"])
            else:
                ds = _open_netcdf_path(_src["path"], _src["mtime"])
            if ds is not None:
                ds = ds.copy(deep=False)   # cached object is shared; never mutate it
            else:
        ...
        if ds is not None:
            ...
            with st.expander("\U0001F5C2\ufe0f  About this data", expanded=True):
...
if not st.session_state.dataset_loaded:
    _LAND_CSS = """
<style>
.pce-hero{ text-align:center; max-width:1180px; margin:3.2vh auto 0; }
.pce-orb{ position:relative; width:92px; height:92px; margin:0 auto 22px;
          display:grid; place-items:center; animation:fadeInUp .7s ease both; }
.pce-orb span{ font-size:60px; line-height:1;
               filter:drop-shadow(0 0 24px rgba(__TEALRGB__,.45));
               animation:pceFloat 6s ease-in-out infinite; }
.pce-ring{ position:absolute; inset:-10px; border-radius:50%;
           border:1.5px dashed rgba(__TEALRGB__,.40);
           animation:pceSpin 18s linear infinite; }
.pce-ring.r2{ inset:-24px; border:1px solid rgba(__TEALRGB__,.14);
              animation-duration:32s; animation-direction:reverse; }
.pce-ring.r2::after{ content:""; position:absolute; top:-4px; left:50%;
                     width:8px; height:8px; border-radius:50%;
                     background:__TEAL__; box-shadow:0 0 12px __TEAL__; }
.pce-eyebrow{ font-size:.74rem; font-weight:600; letter-spacing:.34em;
              text-transform:uppercase; color:__TEAL__; margin-bottom:12px;
              animation:fadeInUp .7s .08s ease both; }
.pce-title{ font-family:'Syne',sans-serif; font-weight:800; line-height:1.05;
            font-size:clamp(1.7rem,5.2vw,4.4rem); letter-spacing:-.02em;
            white-space:nowrap; display:inline-block; max-width:100%;
            background:linear-gradient(100deg,__TEAL__ 0%,#60b4ff 30%,#c89bff 55%,__TEAL__ 100%);
            background-size:220% auto; -webkit-background-clip:text; background-clip:text;
            color:transparent;
            animation:pceShimmer 10s linear infinite, fadeInUp .7s .14s ease both; }
.pce-tag{ font-size:clamp(1.05rem,1.9vw,1.4rem); font-weight:600; color:__TEXT__;
          margin:16px 0 8px; animation:fadeInUp .7s .22s ease both; }
.pce-sub{ font-size:.98rem; line-height:1.7; color:__MUTED__; max-width:640px;
          margin:0 auto; animation:fadeInUp .7s .30s ease both; }

.st-key-pce_land{ max-width:660px; margin:30px auto 0 !important;
                  background:__GLASS__ !important;
                  backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px);
                  border:1px solid rgba(__TEALRGB__,.20) !important;
                  border-radius:20px !important; padding:22px 26px 20px !important;
                  box-shadow:0 18px 50px rgba(0,0,0,__SHADOW__);
                  animation:fadeInUp .7s .40s ease both; }
.pce-act-title{ font-size:1.06rem; font-weight:700; color:__TEXT__; margin-bottom:4px;
                text-align:center; }
.pce-act-sub{ font-size:.84rem; line-height:1.6; color:__MUTED__; margin-bottom:14px;
              text-align:center; }
.st-key-pce_land .stButton > button{ min-height:48px; white-space:normal !important;
                                     justify-content:flex-start !important;
                                     text-align:left !important; }
.st-key-pce_land .stButton > button p{ text-align:left !important; }
.st-key-pce_land button[kind="primary"],
.st-key-pce_land [data-testid="stBaseButton-primary"]{
    background:linear-gradient(135deg,rgba(__TEALRGB__,1),rgba(__TEALRGB__,.78)) !important;
    border:none !important;
    box-shadow:0 6px 22px rgba(__TEALRGB__,.32) !important; }
.st-key-pce_land button[kind="primary"] p,
.st-key-pce_land [data-testid="stBaseButton-primary"] p{ color:__ONTEAL__ !important;
                                                         font-weight:700 !important; }

.pce-feats{ max-width:980px; margin:18px auto 0; display:grid;
            grid-template-columns:repeat(4,1fr); gap:12px;
            animation:fadeInUp .7s .52s ease both; }
.pce-feat{ background:__GLASS__; border:1px solid rgba(__TEALRGB__,.12);
           border-radius:16px; padding:16px 16px 14px;
           backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px);
           transition:transform .25s ease, border-color .25s ease; }
.pce-feat:hover{ transform:translateY(-3px); border-color:rgba(__TEALRGB__,.38); }
.pce-feat .i{ font-size:1.45rem; }
.pce-feat .t{ font-weight:700; font-size:.92rem; color:__TEXT__; margin:6px 0 4px; }
.pce-feat .d{ font-size:.79rem; line-height:1.55; color:__MUTED__; }
.pce-sources{ text-align:center; font-size:.72rem; letter-spacing:.08em;
              color:__MUTED__; margin:22px auto 34px;
              animation:fadeInUp .7s .62s ease both; }
.pce-sources b{ color:__TEXT__; font-weight:600; }

/* the landing page needs no sidebar */
[data-testid="stSidebar"], [data-testid="stSidebarCollapsedControl"],
[data-testid="stExpandSidebarButton"], button[data-testid="collapsedControl"]{
...
@keyframes pceShimmer{ to{ background-position:220% center; } }
@media (max-width:900px){ .pce-feats{ grid-template-columns:repeat(2,1fr); } }
@media (max-width:540px){ .pce-feats{ grid-template-columns:1fr; }
                          .st-key-pce_land{ padding:18px 16px !important; } }
@media (prefers-reduced-motion:reduce){
  .pce-orb, .pce-orb span, .pce-ring, .pce-eyebrow, .pce-title, .pce-tag, .pce-sub,
  .st-key-pce_land, .pce-feats, .pce-sources{ animation:none !important; } }
</style>
"""
    for _tok, _val in (("__TEALRGB__", _teal_rgb), ("__TEAL__", _teal),
                       ("__TEXT__", "#e8f4f8" if DK else "#0c2846"),
                       ("__MUTED__", _text_muted),
                       ("__GLASS__", "rgba(10,20,32,0.58)" if DK else "rgba(255,255,255,0.74)"),
                       ("__SHADOW__", "0.35" if DK else "0.10"),
                       ("__ONTEAL__", "#04121c" if DK else "#ffffff")):
        _LAND_CSS = _LAND_CSS.replace(_tok, _val)
    st.markdown(_LAND_CSS, unsafe_allow_html=True)

    st.markdown("""
<div class="pce-hero">
  <div class="pce-orb"><div class="pce-ring"></div><div class="pce-ring r2"></div><span>\U0001F30D</span></div>
  <div class="pce-eyebrow">Climate intelligence for India</div>
  <div class="pce-title">PyClimaExplorer</div>
  <div class="pce-tag">Climate data for the people who have to act on it.</div>
  <div class="pce-sub">Turn raw NetCDF climate files into maps, long-term trends and a
  plain-language briefing &mdash; for any Indian state, in English or \u0939\u093f\u0902\u0926\u0940,
  online or completely offline.</div>
</div>
""", unsafe_allow_html=True)

    _k = st.session_state.get("_file_uploader_key", 0)
    try:
        _land = st.container(key="pce_land")
    except TypeError:
        _land = st.container()
    with _land:
        st.markdown('<div class="pce-act-title">\U0001F4C2 Upload a NetCDF file to begin</div>'
                    '<div class="pce-act-sub">Any file with latitude, longitude and time '
                    '\u2014 from IMD, NOAA, ERA5 or a climate model.</div>',
                    unsafe_allow_html=True)
        st.file_uploader("Upload a NetCDF (.nc) file", type=["nc"],
                         key=f"land_upload_{_k}", label_visibility="collapsed",
                         on_change=_take_upload, args=(f"land_upload_{_k}",))

    st.markdown("""
<div class="pce-feats">
  <div class="pce-feat"><div class="i">\U0001F9E0</div><div class="t">Insight Briefing</div>
    <div class="d">Every number is computed in Python; AI only writes the sentences. An audit panel shows each figure's source.</div></div>
  <div class="pce-feat"><div class="i">\U0001F5FA\ufe0f</div><div class="t">Any state, in context</div>
    <div class="d">All 31 states and union territories, with their real borders, from a single national file.</div></div>
  <div class="pce-feat"><div class="i">\U0001F4E1</div><div class="t">Works offline</div>
    <div class="d">Ask questions about your data with no API key and no GPU &mdash; built for a block-office laptop.</div></div>
  <div class="pce-feat"><div class="i">\U0001F5E3\ufe0f</div><div class="t">English &amp; \u0939\u093f\u0902\u0926\u0940</div>
    <div class="d">Briefings written for farmers and district officials, not for climate scientists.</div></div>
</div>
<div class="pce-sources">DATA &nbsp;\u00b7&nbsp; <b>India Meteorological Department</b> &nbsp;\u00b7&nbsp;
<b>NOAA Physical Sciences Laboratory</b> &nbsp;\u00b7&nbsp; <b>geoBoundaries</b></div>
""", unsafe_allow_html=True)
                ...
                if time_dim_ds is None:
                    ...
                    if not var_list:
                        ...
                        if tdv and "lat" in data_cmp.dims and "lon" in data_cmp.dims:
                            ...
                            shared = st.toggle(
                                "Same colour scale for both maps", value=True,
                                key="cmp_shared",
                                help="On: a colour means the same value on both sides, "
                                     "so you can compare them directly. Off: each map "
                                     "uses its own range, which shows the pattern inside "
                                     "each one but makes colours incomparable.")
                            if shared:
                                ra = rb = map_range(pa["z"], pb["z"])
                            else:
                                ra, rb = map_range(pa["z"]), map_range(pb["z"])
                            for col, prep, t, tag, k, (vmin, vmax) in (
                                    (col_left, pa, t1, "A", "cmp_chart_a", ra),
                                    (col_right, pb, t2, "B", "cmp_chart_b", rb)):
                                with col:
                                    st.markdown(
                            ...
                            if shared:
                                st.caption("Both maps share one colour scale, so a colour "
                                           "means the same value on each side. If one slice "
                                           "is far wetter or hotter, the other can look "
                                           "flat \u2014 switch the scale off to see each "
                                           "pattern on its own.")
                            else:
                                st.caption("Each map uses its own colour scale. Read the "
                                           "colour bars before comparing: the same colour "
                                           "means different values on each side.")

                            # ---- what changed between the two ------------------
                            pdiff = prep_for(vb - va, lat, lon, reg_c)
                            dmin, dmax = map_range(pdiff["z"], symmetric=True)
                            diff_pal = diverging_scale(
                                "rainfall" if classify_variable(variable_cmp) == "rainfall"
                                else "other")
                            st.markdown(
...
# =================================================================
# THEME TOGGLE — floating, bottom-right. Sits above the chat launcher
# on the dashboard, in the corner on the landing page.
# =================================================================
def _toggle_theme():
...
<style>
.st-key-pce_theme{
    position:fixed !important; right:22px !important; bottom:__BOTTOM__ !important;
    left:auto !important; top:auto !important; width:auto !important;
    z-index:999998 !important; background:transparent !important;
    border:none !important; box-shadow:none !important; padding:0 !important; }
.st-key-pce_theme button{
    width:48px !important; height:48px !important; min-height:48px !important;
    padding:0 !important; border-radius:50% !important;
    display:flex !important; align-items:center !important; justify-content:center !important;
    background:__BG__ !important;
    border:1px solid rgba(__TEALRGB__,0.38) !important;
    backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px);
    box-shadow:0 8px 24px rgba(0,0,0,__SH__) !important;
    transition:transform .25s ease, box-shadow .25s ease !important; }
.st-key-pce_theme button:hover{
    transform:rotate(20deg) scale(1.07);
    box-shadow:0 10px 28px rgba(__TEALRGB__,0.30) !important; }
.st-key-pce_theme button p{ font-size:1.25rem !important; margin:0 !important; line-height:1 !important; }
</style>
"""
for _tok, _val in (("__BOTTOM__", "92px" if st.session_state.get("_src") else "22px"),
                   ("__BG__", "rgba(10,20,32,0.72)" if DK else "rgba(255,255,255,0.88)"),
                   ("__TEALRGB__", _teal_rgb),
                   ("__SH__", "0.40" if DK else "0.12")):
    _theme_css = _theme_css.replace(_tok, _val)
st.markdown(_theme_css, unsafe_allow_html=True)
try:
    _theme_box = st.container(key="pce_theme")
except TypeError:
    _theme_box = st.container()
with _theme_box:
    st.button("☀️" if DK else "🌙", key="mode_toggle",
              on_click=_toggle_theme,
              help="Switch to light mode" if DK else "Switch to dark mode")
