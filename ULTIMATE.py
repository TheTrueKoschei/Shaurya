...
def prepare_map(vals, lat, lon, clip=True):
    """
    Decide how to draw a grid honestly.
      coarse (under 6 cells a side): real cells, no interpolation.
      otherwise: refine so the coast can be traced, then clip to India.
    """
    vals = np.asarray(vals, float)
    lat = np.asarray(lat, float); lon = np.asarray(lon, float)
    ny, nx = vals.shape
    base = dict(ny=ny, nx=nx, lat=lat, lon=lon)
    if ny < 6 or nx < 6:
...
def map_figure(prep, variable, units, palette, vmin, vmax, height=440):
    ...
    lon = prep["lon"]; lat = prep["lat"]
    rest, states, india = _outline_sets(_BOUNDARY_SIG, float(np.nanmax(lon)) > 180.0)
    ...
    if prep["coarse"]:        # show the edge cells in full
...
def map_caption(prep):
    if prep["coarse"]:
        ...
        st.caption(f"Only {prep['ny']}\u00d7{prep['nx']} grid cells cover this area"
    ...
    with st.container(border=True):
        ...
        if "lat" in dims and "lon" in dims:
            try:
                ...
                raw = data.isel(time=t_idx).values if "time" in dims else data.values
                vals, units = display_values(raw, variable, data.attrs.get("units", ""))
                prep = prepare_map(vals, ds["lat"].values, ds["lon"].values)
                        ...
                        if tdv and "lat" in data_cmp.dims and "lon" in data_cmp.dims:
                            ...
                            with col_right:
                                t2 = st.select_slider("Time B", options=opts, value=n_t - 1,
                                                      ...
                                                      key="cmp_t2")
                            lat = ds["lat"].values; lon = ds["lon"].values
                            u_raw = data_cmp.attrs.get("units", "")
                            va, units = display_values(data_cmp.isel({tdv: t1}).values,
                                                       variable_cmp, u_raw)
                            vb, _ub = display_values(data_cmp.isel({tdv: t2}).values,
                                                     variable_cmp, u_raw)
                            pa = prepare_map(va, lat, lon)
                            pb = prepare_map(vb, lat, lon)
                            ...
                            pdiff = prepare_map(vb - va, lat, lon)
            ...
            with st.container(border=True):
                ...
                if not var_list:
                    ...
                    vals_s, units_s = display_values(
                        data_story.isel({time_dim_story: t_idx}).values,
                        variable_story, data_story.attrs.get("units", ""))
                    prep_s = prepare_map(vals_s, lat, lon)
